        ...
        for chapter_id in os.listdir(speaker_path):
            ...
            for file_name in os.listdir(chapter_path):
                if file_name.endswith(".wav") and not file_name.startswith("."):
                    ...
                    entries.append(f"{utt_id}|{os.path.abspath(wav_path)}|{text}")
...
def main():
    ...
    # Write train file
    train_file_path = os.path.join(args.output_dir, "train.raw.list")
    with open(train_file_path, "w", encoding="utf-8") as f:
        for entry in train_entries:
            f.write(f"{entry}\n")

    # Write valid file
    valid_file_path = os.path.join(args.output_dir, "valid.raw.list")

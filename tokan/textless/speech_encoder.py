...
from .f0_preprocess import get_f0, align_f0_to_durations, F0_FRAME_SPACE
...
def get_streams(
    ...
    dense_features = dense_model(waveform, padding)
...
class SpeechEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("bos", torch.tensor([self.unit_vocab_size], dtype=torch.int))
        self.register_buffer("eos", torch.tensor([self.unit_vocab_size + 1], dtype=torch.int))
    ...
    def maybe_resample(self, waveform: torch.Tensor, input_sample_rate: int) -> torch.Tensor:
        ...
        if input_sample_rate == self.expected_sample_rate:
            return waveform
        return torchaudio.functional.resample(waveform, input_sample_rate, self.expected_sample_rate)
